...
- `GET /request-groups/:studentId` → list default + custom request groups for student
- `POST /request-groups` → create custom request group for student
- `GET /request-groups/:studentId/members/:groupName` → list requester identities in a request group
- `POST /request-groups/members` → add requester identity to request group
- `PATCH /request-groups/members/:id/status` → activate/deactivate requester identity
...
All consent grant/revoke and read checks use real Algorand calls to the configured network (LocalNet/TestNet/MainNet), not simulated state.
