...
import { canonicalDefaultDataGroup, normalizeDataGroupName } from '../utils/dataGroups.js'
import { normalizeRequestGroupName } from '../utils/requestGroups.js'

export const documentsRouter = Router()
const upload = multer({ storage: multer.memoryStorage(), limits: { fileSize: 10 * 1024 * 1024 } })

function prepareStoredPayload(fileBase64: string) {
  ...
  return { storageMode, encryptedBlob, plainBlob }
}

async function resolveAllowedDataGroup(studentId: string, rawDataGroup: string) {
  const defaultMatch = canonicalDefaultDataGroup(rawDataGroup)
  if (defaultMatch) return defaultMatch

  const normalized = normalizeDataGroupName(rawDataGroup)
  const custom = await DataGroupModel.findOne({ studentId, normalizedName: normalized }).lean()
...
documentsRouter.post('/upload', async (req, res) => {
  ...
  try {
    const allowedDataGroup = await resolveAllowedDataGroup(String(studentId), String(dataGroup))
    if (!allowedDataGroup) {
    ...
    const { storageMode, encryptedBlob, plainBlob } = prepareStoredPayload(fileBase64)

    const created = await DocumentModel.create({
      studentId,
      receiverGroup,
      dataGroup: allowedDataGroup,
      fileName,
      mimeType,
      sizeBytes,
...
documentsRouter.post('/upload-form', upload.single('file'), async (req, res) => {
...
documentsRouter.get('/download/:id', async (req, res) => {
  ...
  try {
    ...
    if (!ownerAccess) {
      ...
      if (requesterId) {
        const member = await RequesterIdentityModel.findOne({
          _id: requesterId,
          studentId: ownerStudentId,
          requestGroupNormalized: normalizeRequestGroupName(requesterGroup),
          status: 'active',
        }).lean()

        if (!member) {
          return res.status(403).json({
            error: 'requesterId is not an active member of requesterGroup for this student',
          })
        }
      }
...
documentsRouter.get('/:studentId/grouped', async (req, res) => {
  ...
  try {
    ...
    const groups = Array.from(grouped.entries()).map(([dataGroup, documents]) => ({
      dataGroup,
