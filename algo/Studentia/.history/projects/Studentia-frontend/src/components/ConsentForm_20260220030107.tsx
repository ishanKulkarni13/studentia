    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Grant or revoke consent for data sharing. App ID: {appId || "Set VITE_APP_ID"}
          </CardDescription>
        </CardHeader>
            ...
            <ul className="space-y-2">
              {logs.map((log, idx) => (
                <li key={idx} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div>
                    <span className={`font-medium ${log.status === "Granted" ? "text-green-600" : "text-red-600"}`}>
                      {log.status}
                    </span>
                    <span className="text-gray-600 ml-2">
                      {log.receiver} / {log.data}
                    </span>
                  </div>
                  <div className="text-sm text-blue-600">
                    {explorerBase ? (
                      <a href={`${explorerBase}${log.txId}`} target="_blank" rel="noreferrer" className="hover:underline">
                        {log.txId.slice(0, 10)}...
                      </a>
                    ) : (
                      log.txId.slice(0, 10) + "..."
