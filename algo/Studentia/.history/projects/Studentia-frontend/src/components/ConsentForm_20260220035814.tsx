            ...
            <div className="space-y-3">
              {records.map((rec, idx) => (
                <div key={idx} className="flex items-center justify-between p-4 border rounded-lg">
                  <div className="flex items-center gap-3">
                    <Badge variant={
                      rec.status.toLowerCase() === 'granted' ? 'success' :
                      rec.status.toLowerCase() === 'revoked' ? 'error' :
                      'secondary'
                    }>
                      {rec.status}
                    </Badge>
                    <div className="text-sm">
                      <span className="font-medium">{rec.receiver}</span>
                      <span className="text-muted-foreground"> / {rec.data}</span>
                    </div>
                  </div>
                  <div className="text-sm text-muted-foreground">
                    {rec.txId ? (
                      ...
                      <span className="text-muted-foreground">No transaction</span>
                    )}
                  </div>
                </div>
              ))}
