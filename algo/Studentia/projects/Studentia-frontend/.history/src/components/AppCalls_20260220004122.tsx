...
const AppCalls = ({ openModal, setModalState }: AppCallsInterface) => {
  ...
  const refreshOnChainStatus = async (student: string) => {
    if (!apiBase) {
      enqueueSnackbar("Backend API not configured; set VITE_API_BASE for on-chain reads", { variant: "warning" });
      return;
    }
    if (!student.trim()) {
    ...
    try {
      const resp = await fetch(`${apiBase}/consents/onchain/${encodeURIComponent(student.trim())}`, {
        headers: {
          ...(import.meta.env.VITE_API_TOKEN ? { Authorization: `Bearer ${import.meta.env.VITE_API_TOKEN}` } : {}),
        },
      });
    ...
    <dialog id="appcalls_modal" className={`modal ${openModal ? "modal-open" : ""} bg-slate-200`}>
      <form method="dialog" className="modal-box">
        ...
        {onChainStatuses.length > 0 && (
          <div className="mt-4 text-left">
            <div className="font-semibold mb-2">Stored status (on-chain)</div>
            <ul className="space-y-1 max-h-48 overflow-y-auto">
              {onChainStatuses.map((rec, idx) => (
                <li key={`${rec.boxKey}-${idx}`} className="text-sm border rounded p-2">
                  <div>
                    {rec.status} → {rec.receiver} / {rec.data}
                  </div>
                  <div className="text-xs text-blue-700 break-all">Box: {rec.boxKey || "n/a"}</div>
                </li>
              ))}
            </ul>
          </div>
